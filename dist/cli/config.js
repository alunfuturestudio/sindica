...
export async function configureProject(options) {
    ...
    const detectedProjectName = sanitizeName(options.projectName ?? detectedPackage?.name ?? basename(targetDir) ?? "my-project");
...
function printPostConfigInstructions(config) {
...
async function promptForConfig(defaults) {
    ...
    try {
        ...
        const validation = await ask(rl, "Validation commands separated by ;", defaults.validationCommands.join("; "));
        return {
            projectName,
            baseBranch,
            configPath,
...
async function ensureWritableTarget(targetDir, yes) {
    if (yes) {
        return;
    }
    const existing = [
    ...
    if (found.length > 0) {
        throw new Error(`Sindica files already exist: ${found.join(", ")}. Re-run with --yes to overwrite generated files.`);
    }
}
async function writeProjectFiles(targetDir, config) {
...
async function updatePackageScripts(targetDir, config) {
...
async function detectBaseBranch(targetDir) {
    try {
...
function detectValidationCommands(packageJson) {
...
function setupStateFile(config) {
    return `${JSON.stringify({
        version: 1,
...
function dockerRuntimeFiles(config) {
    return [
...
function runtimeCompose(config) {
    return `services:
  runtime:
    build:
      context: ../..
...
function runtimeSindicaRun(config) {
    return `#!/usr/bin/env bash
set -euo pipefail
...
function postConfigReadme(config) {
