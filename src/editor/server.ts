...
import type { Action, AgentConfig, Issue, Pipeline, Rule } from "../core/types.js";
...
async function handleRequest(
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/agent") {
      const body = await readJsonBody<AgentUpdateRequest>(request);
    ...
    if (request.method === "POST" && url.pathname === "/api/rule") {
      const body = await readJsonBody<RuleUpdateRequest>(request);
    ...
    if (request.method === "POST" && url.pathname === "/api/agent/new") {
      const body = await readJsonBody<NewAgentRequest>(request);
    ...
    if (request.method === "POST" && url.pathname === "/api/agent/delete") {
      const body = await readJsonBody<DeleteAgentRequest>(request);
    ...
    if (request.method === "POST" && url.pathname === "/api/phase/new") {
      const body = await readJsonBody<NewPhaseRequest>(request);
...
function portableTemplateContent(content: string): string {
  return content
    .replaceAll('from "../src/index"', 'from "sindica"')
    .replaceAll("from '../src/index'", 'from "sindica"');
}

function templatePathFor(template: string): string {
...
export function updateAgentConfig(request: AgentUpdateRequest): string {
...
export function updateRuleConfig(request: RuleUpdateRequest): string {
  let content = request.content;
  ...
  if (request.targetPhase) {
    block = replaceFirstCall(block, "addLabel", request.targetPhase, (value) =>
...
export function addAgentConfig(request: NewAgentRequest): string {
...
export function addPhaseConfig(request: NewPhaseRequest): string {
  const phase = normalizePhaseId(request.phase);
...
export function deleteAgentConfig(request: DeleteAgentRequest): string {
  const name = request.name.trim();
  if (!name) {
    throw new Error("Missing agent name.");
  }
...
function locateRuleObjectsAssigningAgent(
  content: string,
  agentName: string
...
function locateTopLevelObjects(
  content: string,
  arrayStart: number,
  arrayEnd: number
): { start: number; end: number }[] {
...
function upsertStringProperty(block: string, property: string, value: string): string {
...
function upsertStringArrayProperty(
  block: string,
  property: string,
...
function upsertNumberProperty(block: string, property: string, value: number): string {
...
function upsertExpressionProperty(block: string, property: string, expression: string): string {
...
function replaceFirstCall(
  block: string,
  functionName: string,
  nextValue: string,
  filterPreviousValue: (value: string) => boolean = () => true
): string {
...
function insertPropertyAfterName(block: string, line: string): string {
...
function replaceStringLiteralCalls(
  content: string,
  functionName: string,
  ...
  const pattern = new RegExp(
    `${functionName}\\(\\s*(['"\`])${escapeRegExp(previousValue)}\\1\\s*\\)`,
    "g"
  );
  return content.replace(pattern, `${functionName}(${JSON.stringify(nextValue)})`);
}

function ensureSindicaImports(content: string, names: readonly string[]): string {
...
function insertIntoArrayProperty(content: string, property: string, block: string): string {
...
function removeObjectBlock(content: string, start: number, end: number): string {
...
function uniqueRuleId(content: string, baseId: string): string {
...
function nextRulePriority(content: string): number {
...
function findPropertyValueEnd(content: string, start: number): number {
  let parenDepth = 0;
  let braceDepth = 0;
  let bracketDepth = 0;
  let quote: string | undefined;
...
function topologicalBoxOrder(
  ...
  const edges = rules
    ...
    .map((rule) => ({
      source: rule.phase,
      target: rule.targetPhase as string,
      priority: rule.priority,
    }))
    .sort((left, right) =>
  ...
  for (const edge of edges) {
    if (
      createsPath(outgoing, edge.target, edge.source)
    ) {
      continue;
    }
...
function boxHeight(ruleCount: number, agentCount: number): number {
...
function compareBoxIds(left: string, right: string): number {
...
function boxRank(id: string): number {
  if (id === "status:todo") return 0;
  if (id.startsWith("status:")) return 1;
  if (id === "phase:alexo") return 2;
  if (id.startsWith("phase:")) return 3;
  return 4;
}

function labelNames(pipeline: Pipeline): string[] {
  return [...new Set((pipeline.labels ?? []).map((label) =>
    typeof label === "string" ? label : label.name
  ))];
}

interface RuleConditionInference {
  sourceBox: string;
  state: string;
}

function inferRuleCondition(
  rule: Rule,
  candidates: readonly Issue[]
): RuleConditionInference {
  const matched = candidates.filter((issue) => safeMatch(rule, issue));
  if (matched.length === 0) {
    return inferRuleConditionFromSource(rule);
  }

  const sourcePhases = unique(matched.flatMap((issue) =>
    issue.labels.values.filter((label) => label.startsWith("phase:"))
  ));
...
function inferRuleConditionFromSource(rule: Rule): RuleConditionInference {
...
function safeMatch(rule: Rule, issue: Issue): boolean {
  try {
...
function buildCandidateIssues(labels: readonly string[]): Issue[] {
...
function fakeIssue(status: string, labels: readonly string[]): Issue {
  return {
...
function collapseStatuses(statuses: readonly string[]): string {
...
function normalizeStatusId(status: string): string {
...
function phaseForAgent(agent: AgentConfig, rules: RuleView[]): string | undefined {
  return rules.find((rule) => rule.assignedAgent === agent.name)?.targetPhase;
}

function phaseFromActions(actions: readonly Action[]): string | undefined {
  return actions.find(isPhaseAddLabel)?.label;
}

function isPhaseAddLabel(action: Action): action is Extract<Action, { type: "addLabel" }> {
  return action.type === "addLabel" && action.label.startsWith("phase:");
}

function assignedAgentFromActions(actions: readonly Action[]): string | undefined {
  return actions.find((action) => action.type === "assignAgent")?.agent;
}

function extractPhaseLabels(source: string): string[] {
  return extractStrings(source).filter((value) => value.startsWith("phase:"));
}

function extractStatus(source: string): string {
  const values = extractStrings(source);
  return values.find((value) =>
    ["todo", "TODO", "in_review", "In Review", "a_validar_homolog", "Validating Homolog"].includes(value)
  ) ?? "any";
}
